...
public class PressureGraph {
    ...
    public void pressureGraph(){
        ...
        Vector<Double> pressureVector = readFile.getPressureVector();

        // temperatureVector.addAll(0,readFile.getTemperatureVector());
        //locationVector.addAll(0,readFile.getLocationVector());
...
//        }
        ...
        int counter2 = 0;
        int counter3 = 0;
        int n=pressureVector.size();

        for (int i =n-50; i < n; i++) {
            if (locationVector.get(i).contains("pysaxion")) {
                series1.getData().add(new XYChart.Data(counter, pressureVector.get(i)));
                counter++;
            } else if (locationVector.get(i).contains("pygronau")) {
                series2.getData().add(new XYChart.Data(counter1, pressureVector.get(i)));
                counter1++;
            } else if (locationVector.get(i).contains("pywierden")) {
                series3.getData().add(new XYChart.Data(counter2, pressureVector.get(i)));
                counter2++;
            } else if (locationVector.get(i).contains("pygarage")) {
                series4.getData().add(new XYChart.Data(counter3, pressureVector.get(i)));
                counter3++;
            }
        }
